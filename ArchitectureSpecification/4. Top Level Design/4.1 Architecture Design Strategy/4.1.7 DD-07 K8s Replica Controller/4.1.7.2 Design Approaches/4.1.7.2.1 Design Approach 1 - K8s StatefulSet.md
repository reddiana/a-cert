...
- **순차적 롤링 배포 (`OrderedReady` Policy) 및 리더 폭포 장애 방지 (`DD-01` 연계):**
  ...
  - 클러스터의 리더 노드(`batch-scheduler-0`)는 모든 팔로워 노드가 먼저 배포 완료되어 정상 가동된 후 마지막에 업데이트되므로, 배포 도중 리더 동시 다운에 따른 RDBMS 락 경합 및 리더 재선출 파동(Flapping) 없이 3초 자가 치유 SLA를 안정적으로 보장합니다.
...
**보완 메커니즘**
- **StatefulSet의 PV/PVC 마운트 작성 복잡도 및 스토리지 구속 → DD-05 및 DD-06 채택 Approach 연계 보완:**
  - 일반적인 StatefulSet 적용 시 전용 볼륨 마운트 스펙을 작성해야 하는 부담은 본 설계에서 채택된 `DD-05 Design Approach 1 (Shared RDBMS Metatable)` 및 `DD-06 Design Approach 1 (Embedded Library Pattern)`과 연계하여 영속 상태를 중앙 DB로 분리함으로써 보완합니다. 별도 PV/PVC 볼륨 마운트 없이 무상태(Stateless-like) 형태로 StatefulSet을 배포하여 고정 인덱스(`batch-0`)와 `OrderedReady` 롤링 배포 장점을 온전히 확보합니다.
- **롤링 배포 시 수 초간의 리더 승계 지연 → DD-04 채택 Approach 연계 보완:**
  - 리더 Pod(`batch-0`)가 순차 배포의 마지막 단계에서 재기동될 때 발생하는 일시적인 리더십 공백은 본 설계에서 채택된 `DD-04 Design Approach 1 (RDBMS Execution Journaling and Dynamic Replay)`과 연계하여 보완합니다. 롤링 업데이트 도중에도 실행 중이던 배치 저널이 DB에 보존되므로, 배포 완료 후 신규 인스턴스가 미완료 작업을 식별하여 즉시 재개합니다.
