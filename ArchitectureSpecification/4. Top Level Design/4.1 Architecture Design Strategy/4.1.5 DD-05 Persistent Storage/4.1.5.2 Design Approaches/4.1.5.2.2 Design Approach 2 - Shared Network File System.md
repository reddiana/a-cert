...
**보완 메커니즘**
- **POSIX 파일 락 지연 및 Stale Lock 위험 → DD-02 채택 Approach 연계 검토 및 한계:**
  - 네트워크 파일 시스템의 POSIX 락 지연 및 노드 다운 시 Stale Lock 문제는 본 설계에서 채택된 `DD-02 Design Approach 2 (DB Record-based Lease 락)`와 같은 RDBMS 기반 Lease 메커니즘을 적용하지 않는 한, NFS 파일 락 자체로는 해소하기 어렵습니다. 이에 따라 해당 Approach 2는 최종 채택되지 않았습니다.
- **ReadWriteMany 스토리지 마운트 구속 오버헤드 → DD-07 채택 Approach 연계 검토 및 한계:**
  - K8s 배포 환경에서 ReadWriteMany PV/PVC 볼륨 마운트 작성 오버헤드는 본 설계에서 채택된 `DD-07 Design Approach 1 (K8s StatefulSet)` 매니페스트와의 결합으로 표준화를 시도할 수 있으나, 별도 스토리지 클래스 구축 및 인프라 변경 최소화 지침(`NR-03`, `NR-08`, `CR-02`)을 충족하지 못하여 해당 대안의 스토리지 인프라 종속성은 해소되지 않습니다.
