**개요**
각 노드가 로컬 디스크에 WAL(Write-Ahead Log) 기반 임베디드 저널 파일을 운영하고, Raft Log를 통해 피어 노드와 상태 변경 이력을 복제·동기화하는 방식입니다. 메타 DB는 최종 영속화(Write-Behind)의 대상으로만 활용되며, DB 장애 중에는 로컬 WAL과 Raft Log만으로 분산 합의 및 동시성 제어 상태를 자립 운영합니다.

![[4.1.5.2.3 Embedded Local Disk WAL Overview.svg]]

![[4.1.5.2.3 Design Approach 3 - Embedded Local Disk WAL and Peer Sync.png]]
...
- **4.1 `notifyCommit(entry={index=401})`** (`Raft Peer Synchronizer` → `Write-Behind Synchronizer`)
    - 커밋 완료를 비동기 DB 동기화 모듈에 통보합니다. 동기화 모듈은 별도 메모리 버퍼 없이 로컬 WAL의 커밋 구간을 판독하여 반영합니다.
...
- **5.3 `bulkSyncReconciliation(gapEntries)`** (`Reconciliation Engine` → `Shared Meta RDBMS`)
    - 누락된 51개의 상태 변경 엔트리를 단일 JDBC 일괄 트랜잭션으로 RDBMS에 반영합니다.
...
- **Raft 통신 포트 추가 개방:** DD-01(리더 선출), DD-02(분산 락), DD-03(분산 큐), DD-04(저널)와 동일 Raft 포트를 공유하여 사용하므로 별도 추가 포트는 최소화되나, K8s Headless Service 적용이 필요함.
