...
**보완 메커니즘**
- **중앙 RDBMS I/O 트래픽 집중 부하 → DD-08 및 DD-01 채택 Approach 연계 보완:**
  - 중앙 RDBMS로 메타 트래픽이 집중되는 부하는 본 설계에서 채택된 `DD-08 Design Approach 1 (Lightweight Custom RDBMS Coordinator Engine)` 및 `DD-01 Design Approach 1 (RDBMS Lock-based 리더 선출)`과 결합하여 보완합니다. 노드 런타임에는 불필요한 DB 조회를 배제하고 3초 주기의 경량 Heartbeat 쿼리만 수행하도록 제어하여 DB I/O 오버헤드를 경감합니다.
