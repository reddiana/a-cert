...
**보완 메커니즘**
- **단일 리더 부재로 인한 중복 스케줄링 리스크 → DD-02 채택 Approach 연계 검토 및 한계:**
  - 리더 노드를 선출하지 않는 구조상 동일 배치가 다중 노드에서 중복 기동되는 문제는 본 설계에서 채택된 `DD-02 Design Approach 2 (DB Record-based Lease 락)`를 적용하여 태스크 수준의 상호 배제를 시도할 수 있습니다. 그러나 리더 중심의 스케줄링 및 분산 큐 통제 기능이 부재하므로, 해당 Approach 2는 최종 채택되지 않았습니다.
- **노드 간 소켓 포트 통신 오버헤드 → DD-07 채택 Approach 연계 검토 및 한계:**
  - P2P 소켓 포트 개방 및 디스커버리 관리 복잡성은 본 설계에서 채택된 `DD-07 Design Approach 1 (K8s StatefulSet)`의 Headless Service DNS 라우팅을 연계하여 관리를 시도할 수 있습니다. 그러나 추가 통신 포트 개방 및 네트워크 변경 최소화 지침(`NR-08`, `CR-02`)을 충족하지 못하여 해당 대안의 근본적인 네트워크 오버헤드는 해소되지 않습니다.
