...
![[4.1.2.2.3 Raft FSM Distributed Lock Overview.png]]

**분산 락 획득 (Lock Acquire) 및 Quorum 합의 단계**

![[4.1.2.2.3 Lock Acquire and Quorum.png]]
...
![[4.1.2.2.3 Follower Lock FSM Sync.png]]
...
- **DB 장애 내구성 충족 (NR-09, QAS-09):** `DD-05 Design Approach 3 (Embedded Local Disk WAL and Peer Sync)` 연계를 통해 메타 DB 장기 장애 상황에서도 Raft 채널 및 로컬 디스크 WAL 영속화만으로 분산 락 라이프사이클을 무중단 제어하여 배치 중복 실행을 차단함.
