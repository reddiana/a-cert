...
![[ArchitectureSpecification/1. Project Overview/1.1 AS-IS Architecture Concept.svg]]
...
![[ArchitectureSpecification/1. Project Overview/1.1 AS-IS Distributed Queue Concept.svg]]

![[1.1 AS-IS Distributed Queue Flow.png]]
...
![[ArchitectureSpecification/1. Project Overview/1.1 AS-IS Distributed Lock Concept.svg]]


![[1.1 AS-IS Distributed Lock Flow.png]]
...
- **개선 대상 (In-Scope):**
  - **분산 코디네이션 및 동기화 제어 엔진**: 외부 분산 클러스터(Hazelcast) 의존성을 배제하고, JDK 표준 API를 활용해 리더 선출, 멤버십 동적 관리, 분산 락(중복 실행 방지), 분산 큐(작업 누락 방지) 기능을 제공하도록 개편합니다.
