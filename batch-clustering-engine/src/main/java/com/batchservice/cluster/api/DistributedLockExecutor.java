...
import com.batchservice.cluster.fsm.LockStateMachine;
 ...
 *
 * <ul>
 *   <li>소유자 식별자는 {@code "{nodeId}:{ownerId}"} 형식으로 기록되어 노드 장애 시 회수 대상 식별이 가능</li>
...
public class DistributedLockExecutor {
    ...
    public <T> T execute(String lockName, String ownerId, long waitMs, long leaseMs, Supplier<T> supplier) {
        String owner = raftNode.getNodeId() + ":" + ownerId;
     ...
     *
     * @throws IllegalStateException waitMs 내에 획득하지 못한 경우
     */
    public long acquire(String lockName, String owner, long waitMs, long leaseMs) {
        long deadline = System.currentTimeMillis() + waitMs;
        long commitTimeout = Math.max(waitMs, raftNode.getTimings().clientTimeoutMs());
